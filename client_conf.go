...
func (cfg *ClientConf) SetAuthMode(authMode string) {
	...
	case "oidc":
		cfg.AuthMode = OIDC
	default:
		log.Printf("authMode value '%s' not recognised, defaulting to 'basic' authentication", authMode)
		cfg.AuthMode = Basic
	}
}
...
func (cfg *ClientConf) getAuthToken() (string, error) {
	...
	case None:
		return "", nil
	default:
		log.Printf("no authentication mode identified, defaulting to none")
		return "", nil
	}
}
...
func checkConf(cfg *ClientConf) error {
	...
	if !strings.HasPrefix(strings.ToLower(cfg.BaseURI), "http") {
		log.Printf("no protocol defined for Onix URI '%s', 'http://' will be added to it", cfg.BaseURI)
...
func (cfg *ClientConf) bearerToken(tokenURI string, clientId string, secret string, user string, pwd string) (string, error) {
	...
	payload := strings.NewReader(
		fmt.Sprintf("grant_type=password&username=%s&password=%s&scope=openid%%20interlink", user, pwd))
